...
def club_database_url() -> Iterator[SecretStr]:
...
async def club_engine(club_database_url: SecretStr) -> AsyncIterator[AsyncEngine]:
    engine = build_async_engine(club_database_url)
    try:
        yield engine
    finally:
