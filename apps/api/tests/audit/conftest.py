...
def audit_database_url() -> Iterator[SecretStr]:
...
async def audit_engine(audit_database_url: SecretStr) -> AsyncIterator[AsyncEngine]:
    engine = build_async_engine(audit_database_url)
    try:
        yield engine
    finally:
...
@pytest_asyncio.fixture
async def audit_session(audit_engine: AsyncEngine) -> AsyncIterator[AsyncSession]:
    factory = async_sessionmaker(audit_engine, class_=AsyncSession, expire_on_commit=False)
