...
def profile_database_url() -> Iterator[SecretStr]:
...
async def profile_engine(profile_database_url: SecretStr) -> AsyncIterator[AsyncEngine]:
    engine = build_async_engine(profile_database_url)
    try:
        yield engine
    finally:
...
@pytest_asyncio.fixture
async def profile_session(profile_engine: AsyncEngine) -> AsyncIterator[AsyncSession]:
